...
def train_val(config):
    ...
    model = Doc2Hash(dataset, num_features, bit, dropoutProb=0.1)
    model.cuda()
    ...
    logging.basicConfig(
        ...
        filename='../../logs/Doc2Hash/data:{}_bit:{}.log'.format(config["dataset"], config["bit"]),
        filemode='w'
    )
    ...
    for epoch in tqdm(range(config["epoch"])):
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                step_count = 0
                torch.save(model, '../../checkpoints/Doc2Hash/dataset:{}_bit:{}.pth'.format(dataset, bit))
            else:
                step_count += 1
    ...
    model = torch.load('../../checkpoints/Doc2Hash/dataset:{}_bit:{}.pth'.format(dataset, bit))
    model.eval()
    with torch.no_grad():
        train_b, test_b, train_y, test_y = model.get_binary_code(train_loader, test_loader)
        retrieved_indices = retrieve_topk(test_b.cuda(), train_b.cuda(), topK=100)
