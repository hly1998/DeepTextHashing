...
def train_val(config):
    ...
    model = NASH(num_features, bit, dropoutProb=0.1)
    model.cuda()
    ...
    logging.basicConfig(
        ...
        filename='../../logs/NASH/data:{}_bit:{}.log'.format(config["dataset"], config["bit"]),
        filemode='w'
    )
    ...
    for epoch in tqdm(range(config["epoch"])):
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                step_count = 0
                torch.save(model, '../../checkpoints/NASH/dataset:{}_bit:{}.pth'.format(dataset, bit))
            else:
                step_count += 1
    ...
    model = torch.load('../../checkpoints/NASH/dataset:{}_bit:{}.pth'.format(dataset, bit))
    model.eval()
    with torch.no_grad():
