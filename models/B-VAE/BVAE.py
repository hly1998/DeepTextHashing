...
class BVAE(nn.Module):
    def __init__(self, vocabSize, latentDim, device, bn=True, tau_ann=False, beta=0):
    ...
    def bkl_loss(logits_b):
        ...
        Nb = logits_b.size(1)
        eps = 1e-20
        kl = Nb * torch.log(torch.tensor(2.0)) + torch.sum(p_b * torch.log(p_b + eps) +
                                                           (1 - p_b) * torch.log(1 - p_b + eps), dim=1)
        return kl
...
def train_val(config):
    ...
    model = BVAE(num_features, bit, device)
    model.to(device)
