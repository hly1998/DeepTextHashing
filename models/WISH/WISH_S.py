...
class WISH_S(nn.Module):
    ...
    def forward(self, document_mat, isStochastic, integration='sum'):
        ...
        score_c = self.pred(z)
        return prob_w, mu, torch.mm(self.topicBook, self.topicBook.t()), score_c
...
def train_val(config):
    ...
    model = WISH_S(dataset, num_features, num_bits, 100, num_bits, y_dim, dropoutProb=0.2)
    model.cuda()
    ...
    logging.basicConfig(
        ...
        filename='../../logs/WISH_S/data:{}_bit:{}.log'.format(config["dataset"], config["bit"]),
        filemode='w'
    )
    ...
    for epoch in tqdm(range(config["epoch"])):
        ...
        for step, (xb, yb) in enumerate(train_loader):
            ...
            kl_loss = WISH_S.calculate_KL_loss(mu)
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                step_count = 0
                torch.save(model, '../../checkpoints/WISH_S/dataset:{}_bit:{}.pth'.format(dataset, bit))
            else:
                step_count += 1
    ...
    model = torch.load('../../checkpoints/WISH_S/dataset:{}_bit:{}.pth'.format(dataset, bit))
    model.eval()
    with torch.no_grad():
        train_b, test_b, train_y, test_y = model.get_binary_code(train_loader, test_loader, isStochastic=True)
        retrieved_indices = retrieve_topk(test_b.cuda(), train_b.cuda(), topK=100)
