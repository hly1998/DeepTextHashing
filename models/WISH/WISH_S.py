...
class WISH_S(nn.Module):
    def __init__(self, dataset, vocabSize, latentDim, topicDim, topicNum, num_classes, device, dropoutProb=0.):
        ...
        self.topicBook = nn.Parameter(torch.rand(self.topicNum, self.topicDim, requires_grad=True))
...
def train_val(config):
    ...
    model = WISH_S(dataset, num_features, num_bits, 100, num_bits, y_dim, device, dropoutProb=0.2)
    model.to(device)
    ...
    alpha = 1.
    topicNum = num_bits
    I_matrix = torch.eye(topicNum).to(device)
    pred_weight = 0.
