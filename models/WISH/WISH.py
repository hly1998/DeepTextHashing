...
class WISH(nn.Module):
    def __init__(self, dataset, vocabSize, latentDim, topicDim, topicNum, device, dropoutProb=0.):
        ...
        self.h_to_mu = nn.Sequential(nn.Linear(self.hidden_dim, self.latentDim), nn.Sigmoid())
        self.decoder = nn.Sequential(nn.Linear(self.topicDim, self.vocabSize), nn.LogSoftmax(dim=1))
...
def train_val(config):
    ...
    model = WISH(dataset, num_features, num_bits, 100, num_bits, device, dropoutProb=0.2)
    model.to(device)
