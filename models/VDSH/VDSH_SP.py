...
def train_val(config):
    ...
    model = VDSH_SP(dataset, num_features, bit, y_dim, dropoutProb=0.1)
    model.cuda()
    ...
    logging.basicConfig(
        ...
        filename='../../logs/VDSH_SP/data:{}_bit:{}.log'.format(config["dataset"], config["bit"]),
        filemode='w'
    )
    ...
    for epoch in tqdm(range(config["epoch"])):
        ...
        for step, (xb, yb) in enumerate(train_loader):
            ...
            # kl_loss = VDSH_SP.calculate_KL_loss(mu, logvar)
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                step_count = 0
                torch.save(model, '../../checkpoints/VDSH_SP/dataset:{}_bit:{}.pth'.format(dataset, bit))
            else:
                step_count += 1
    ...
    model = torch.load('../../checkpoints/VDSH_SP/dataset:{}_bit:{}.pth'.format(dataset, bit))
    model.eval()
    with torch.no_grad():
