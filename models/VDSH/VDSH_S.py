...
class VDSH_S(nn.Module):
    ...
    def forward(self, document_mat):
        ...
        score_c = self.pred(z)
        return prob_w, mu, logvar, score_c
...
def train_val(config):
    ...
    model = VDSH_S(dataset, num_features, bit, y_dim, dropoutProb=0.1)
    model.cuda()
    ...
    logging.basicConfig(
        ...
        filename='../../logs/VDSH_S/data:{}_bit:{}.log'.format(config["dataset"], config["bit"]),
        filemode='w'
    )
    ...
    for epoch in tqdm(range(config["epoch"])):
        ...
        with torch.no_grad():
            ...
            if prec.item() > best_precision:
                ...
                step_count = 0
                torch.save(model, '../../checkpoints/VDSH_S/dataset:{}_bit:{}.pth'.format(dataset, bit))
            else:
                step_count += 1
    ...
    model = torch.load('../../checkpoints/VDSH_S/dataset:{}_bit:{}.pth'.format(dataset, bit))
    model.eval()
    with torch.no_grad():
